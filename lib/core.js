...
const loadApplications = callback => {
  ...
  const serverConfig = impress.config.sections.servers[impress.serverName];
  if (serverConfig && serverConfig.applications) {
    impress.workerApplications = serverConfig.applications;
    createApplications(callback);
  } else {
...
const prepareServersConfig = () => {
  const servers = impress.config.sections.servers;
  impress.config.sections.servers = {};
  for (const serverName in servers) {
    ...
    for (let i = 0; i < server.ports.length; i++) {
      ...
      impress.config.sections.servers[serviceName] = srv;
    }
  }
};
...
impress.start = () => {
  ...
  api.metasync.sequential([
    cb => impress.createSandbox(impress, cb),
    cb => impress.config.loadSchemas(cb),
    cb => impress.config.loadConfig(cb),
  ], () => {
    prepareServersConfig();
    impress.nodeId = impress.config.sections.scale.server +
      'N' + impress.workerId;
    impress.initLogger();
    impress.log.on('open', () => {
      preload();
...
const configureServer = server => {
  ...
  if (server.instance.setTimeout) {
    server.instance.keepAlive = keepAlive;
    server.instance.setTimeout(timeout, serverOnTimeout);
  }
...
impress.load = () => {
  ...
  loadApplications(() => {
    ...
    if (!impress.isMaster && !impress.workerApplications) {
      const configServers = impress.config.sections.servers;
      for (const key in configServers) {
        const serverConfig = configServers[key];
        const server = impress.servers[key];
        if (server && serverConfig.protocol === 'jstp' && server.instance) {
  ...
  if (typeof global.gc === 'function' && impress.config.sections.scale.gc > 0) {
    setInterval(global.gc, impress.config.sections.scale.gc);
  }
...
impress.loadCertificates = serverConfig => {
  if (serverConfig.key && serverConfig.cert) {
    const certDir = api.path.join(impress.dir, 'config/ssl');
    const keyFile = api.path.join(certDir, serverConfig.key);
    const certFile = api.path.join(certDir, serverConfig.cert);
    try {
