...
const MEMORY_LIMIT_CHECK_INTERVAL = 60000;
...
if (impress.memoryLimit) {
  api.timers.setInterval(() => {
    ...
    if (memory.heapTotal < impress.memoryLimit) return;
    impress.cache.clear();
    ...
    if (memory.heapTotal > impress.memoryLimit) {
      const heap = api.common.bytesToSize(memory.heapTotal);
      fatalError(new Error(`Memory limit exceeded: ${heap}, restarting`));
    }
  }, MEMORY_LIMIT_CHECK_INTERVAL);
}

const loadPlugins = () => {
  CORE_PLUGINS.forEach(name => {
    const plugin = {};
    impress[name] = plugin;
    require(`./impress.${name}.js`);
    if (plugin.mixImpress) plugin.mixin(impress);
  });
};

const mixinPlugins = (application) => {
  CORE_PLUGINS.forEach(name => {
    const plugin = impress[name];
    application[name] = {};
...
const loadApplications = (callback) => {
  ...
  if (server && server.applications) {
    ...
    api.fs.readdir(impress.applicationsDir, (err, apps) => {
      if (err) {
        fatalError(impress.CANT_READ_DIR + impress.applicationsDir);
        callback();
      } else {
...
const loadServer = () => {
  ...
  if (!impress.workerApplicationName) {
    ...
    console.log('>>>>>>>>>>' + impress.nodeId);
  }
  // Set garbage collection interval
  const interval = impress.config.scale.gc;
  if (global.gc &&  interval> 0) {
    api.timers.setInterval(gc, interval);
  }
...
impress.startServers = (
  ...
  impress.serversCount = names.length;
  ...
  for (const name in servers) {
    ...
    if (process.isMaster) {
      if (name === 'master') {
        impress.createServer(server);
      } else if (impress.firstStart) {
        impress.forkWorker(workerId++, server);
      }
    } else if (name === impress.serverName) {
      impress.createServer(server);
    }
  }
};

impress.createServer = (
  ...
  impress.setListenerError(server.instance, server.address + ':' + server.port);
...
const checkStarted = () => {
  ...
  api.http.get(cfg.check, (res) => {
    fatalError(new Error(ALREADY_STARTED));
  }).on('error', callback);
};
